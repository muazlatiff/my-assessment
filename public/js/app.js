...
window.axios.defaults.headers.common['X-Requested-With'] = 'XMLHttpRequest'; // for authenticated request

if (document.querySelector('meta[name="token"]')) {
  window.axios.defaults.headers.common['Authorization'] = "Bearer ".concat(document.querySelector('meta[name="token"]').content);
}
...
window.buildErrorMessage = function (errorResponse) {
  var msg = '';

  if (errorResponse.errors) {
    for (var errName in errorResponse.errors) {
      msg += "<b class=\"text-red-600\">".concat(errName, "</b> : ").concat(errorResponse.errors[errName][0], "<br>");
    }
  } else {
    msg = errorResponse.message ? errorResponse.message : 'Something went wrong.<br>';
  }

  return msg;
};

window.preloadElement = function (_element) {
  var toShow = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  var loaderID = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '#loader';

  var _loader = document.querySelector(loaderID);
